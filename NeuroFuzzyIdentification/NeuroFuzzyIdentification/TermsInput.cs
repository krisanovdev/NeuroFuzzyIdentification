...
{
    public partial class TermsInput : Form
    {
        ...
        public TermsInput(int variablesCount, MainForm form, MainFunction mainFunction)
        {
            InitializeComponent();
            delegateObj = form;
        ...
        private DataGridView formattedGridView()
        {
            ...
            dataGrid.Anchor = (AnchorStyles.Bottom | AnchorStyles.Left | AnchorStyles.Top);
            return dataGrid;
        }
        ...
        void BuildFunctionButtonHandler(object sender, EventArgs e)
        {
            ...
            var functions = newValues[i].ConvertAll(x => x.membershipFunction).ToList();
                //variables[i].terms.ConvertAll(x => x.membershipFunction.membershipFunction).ToList();
        ...
        private void dataGridViewTerms_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            var dataGridView = (DataGridView)sender;
            var tag = (int)dataGridView.Tag;

            if (e.ColumnIndex < 0 || e.ColumnIndex >= dataGridView.ColumnCount || e.RowIndex < 0 || e.RowIndex >= dataGridView.RowCount)
                return;
            if (e.ColumnIndex == 0)
            {
                var row = e.RowIndex;
                ...
                MembershipFunctionType membershipType;
                int parametersNumber;
                var cell = (DataGridViewComboBoxCell)dataGridView[0, row];
                var type = dataGridView[0, row].Value.ToString();
                membershipType = (MembershipFunctionType)cell.Items.IndexOf(cell.Value);
                parametersNumber = membershipType.parametersCount();
                var parameters = new List<double>();
                for (var k = 0; k < parametersNumber; k++)
                {
                    try
                    {
                        var param = initialTerms[tag][row].parameters[k];// Convert.ToDouble(dataGridView[1, row].Value);
                        parameters.Add(param);
                    }
                    catch
                    {
                ...
                initialTerms[tag][row] = new MembershipFunction(membershipType, parameters);
                UpdateDataGridViewsTermsParams();
            }
        }
            ...
            if (initialTerms.Count > 0)
            {
                ...
                if (curNumberOfGrids < termsCountArray.Length)
                {
                    for (var i = curNumberOfGrids; i < termsCountArray.Length; i++)
                    {
                        ...
                        newGrid.CellValueChanged += new DataGridViewCellEventHandler(this.dataGridViewTerms_CellValueChanged);
                    ...
                    for (var i = curNumberOfGrids; i < termsCountArray.Length; i++)
                    {
                        ...
                        while (curGrid.RowCount < TermsCount[i])
                        {
                            ...
                            for (var k = 1; k <= maxCount; k++)
                            {
                                ...
                                {
                                    curGrid[k, index].Value = "Відсутній";
                ...
                for (var i = 0; i < termsCountArray.Length; i++)
                {
                    ...
                    if (curGrid.ColumnCount > 0)
                    {
                        ...
                        while (curGrid.RowCount < TermsCount[i])
                        {
                            curGrid.Rows.RemoveAt(curGrid.Rows.Count - 1);
                        }
                    }
                }
        ...
        private void dataGridViewTermsCount_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            var copyInitial = initialTerms.Clone();
            if (copyInitial.Count > 0)
            {
                var termsCountCopy = TermsCount;
                bool isChanged = false;
                for (var i = 0; i < termsCountCopy.Length; i++)
                {
                    if (termsCountCopy[i] > copyInitial[i].Count)
                    {
                        for (var j = copyInitial[i].Count; j < termsCountCopy[i]; j++)
                        {
                            var parameters = new List<double>();
                            parameters.Add(1);
                            parameters.Add(1);
                            var membershipFunction = new MembershipFunction(MembershipFunctionType.bell, parameters);
                            copyInitial[i].Add(membershipFunction);
                            isChanged = true;
                        }
                    }
                    if (termsCountCopy[i] < copyInitial[i].Count)
                    {
                        copyInitial[i].RemoveRange(copyInitial[i].Count - 1, copyInitial[i].Count - termsCountCopy[i]);
                        isChanged = true;
                    }
                }
                if (isChanged)
                {
                    this.initialTerms = copyInitial;
                }
            }
        }

        private void flowLayoutPanelTermsGrids_Paint(object sender, PaintEventArgs e)
        {
